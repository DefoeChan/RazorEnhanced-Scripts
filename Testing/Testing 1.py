    ...
    if Target.HasTarget():
        ...
        while not Mobiles.FindBySerial(enemy.Serial) == None:
            if IsWarrior or IsArcher:
                Pathfinder(enemy.Position.X, enemy.Position.X)
                Misc.Pause(1000)
#            if IsArcher:
...
def Pathfinder(dst_X, dst_Y):
    ...
    route.DebugMessage = True
    route.UseResync = False

    if PathFinding.Go(route):
        Misc.SendMessage("Arrived",170)
    else:
        Misc.SendMessage("Not Arrived", 138)
...
def pathToMobile(mob):
    ...
    mobile = Mobiles.FindBySerial(mob.Serial)
    ...
    #PathFinding.RunPath(PathFinding.GetPath(mobileCoords.X, mobileCoords.Y, 0), 5000, 1, 1)
